...
print(f"\n📁 Hybrid ANN+FAM results saved to: {output_path}")

# ================================
# 🔄 Loop over 10 Runs Automatically
# ================================
all_runs = []
...
for run in range(10):
    ...
    for _ in range(3):
        ...
        extractor = Model(
            ...
            outputs=ann_model.get_layer("feature_layer").output
        )
        ensemble_models.append((ann_model, fam_model, extractor))

    # 3) Train each on fold-0 and collect features
    scaler0      = StandardScaler().fit(X_folds[0])
    X0_scaled    = scaler0.transform(X_folds[0])
    y0           = y_folds[0]
    fold0_feats  = []

    for ann_model, fam_model, extractor in ensemble_models:
        ann_model.fit(X0_scaled, y0, epochs=20, batch_size=32, verbose=0)
        f0 = extractor.predict(X0_scaled, verbose=0)
        fold0_feats.append(f0)

    # 4) Fit one global MinMax on all Fold-0 features
    all_f0       = np.vstack(fold0_feats)
    feat_scaler  = MinMaxScaler().fit(all_f0)

    # 5) Train each FAM using that same scaler
    for (_, fam_model, _), f0 in zip(ensemble_models, fold0_feats):
        fam_model.train(feat_scaler.transform(f0), y0, epochs=1)

    # 6) Online test→train for folds 1–9
    performance = []
    for i in range(1, 10):
        X_test = scaler0.transform(X_folds[i])
        y_test = y_folds[i]

        # 6a) predict with FAM ensemble
        all_preds  = []
        all_scores = []
        for ann_model, fam_model, extractor in ensemble_models:
            # 1) extract features
            feats_i = extractor.predict(X_test, verbose=0)
            feats_i = feat_scaler.transform(feats_i)

            # 2) get both the hard label and the continuous match score
            p_i, s_i = fam_model.predict(feats_i)
        ...
        maj = mode(all_preds, axis=0).mode.flatten()
        avg_score = all_scores.mean(axis=0)

        # 6b) record metrics from the *FAM* match‐score
        auc = roc_auc_score(y_test, avg_score)
        ...
        performance.append({
            ...
            "Recall":    recall_score(y_test, maj),
            "F1":        f1_score(y_test, maj),
            "AUROC":     auc,
            "MCC":       matthews_corrcoef(y_test, maj),
            "TP": tp, "FP": fp, "TN": tn, "FN": fn
        })

        # 6c) Train on this fold
        for ann_model, fam_model, extractor in ensemble_models:
            ann_model.fit(X_test, y_test, epochs=20, batch_size=32, verbose=0)
            tr = extractor.predict(X_test, verbose=0)
            fam_model.train(feat_scaler.transform(tr), y_test, epochs=1)

    # 7) Collect this run’s DataFrame
    all_runs.append(pd.DataFrame(performance))

# 9) Save combined & averaged results
ts       = datetime.now().strftime("%Y%m%d_%H%M%S")
out_xlsx = os.path.expanduser(f"~/Desktop/Hybrid_ANN_FAM_10Runs_{ts}.xlsx")
with pd.ExcelWriter(out_xlsx) as writer:
    for idx, df_run in enumerate(all_runs, start=1):
        df_run.to_excel(writer, index=False, sheet_name=f"Run_{idx:02d}")
    combined_df = pd.concat(all_runs, ignore_index=True)
    avg_df      = combined_df.groupby("Fold").mean().drop(columns=["Run"])
    combined_df.to_excel(writer, index=False, sheet_name="All_10_Runs")
    avg_df.to_excel(writer, index=True,  sheet_name="Average_Performance")
print(f"\n✅ 10 Runs Completed → {out_xlsx}")

'''# ================================
# 🔥 Grid-search α + ρ (no error pool)
# ================================
grid_results = []
alpha_values = np.arange(0.2, 1.2, 0.2)
rho_values   = np.arange(0.0, 1.2, 0.2)

for alpha in alpha_values:
    for rho in rho_values:
        print(f"\n🚀 Grid α={alpha:.2f}, ρ={rho:.2f}")
        per_run_avgs = []
        ...
        for run in range(10):
            ...
            # build fresh ensemble
            ensemble = []
            for _ in range(3):
                ann_m = build_ann(X_folds[0].shape[1])
                fam_m = FuzzyARTMAP(alpha=alpha, rho=rho)
                ext   = Model(
                    inputs=ann_m.input,
                    outputs=ann_m.get_layer("feature_layer").output
                )
                ensemble.append((ann_m, fam_m, ext))

            # fold-0 standardize & ANN→features
            scaler0      = StandardScaler().fit(X_folds[0])
            X0, y0       = scaler0.transform(X_folds[0]), y_folds[0]
            fold0_feats  = []
            for ann_m, fam_m, ext in ensemble:
                ann_m.fit(X0, y0, epochs=20, batch_size=32, verbose=0)
                f0 = ext.predict(X0, verbose=0)
                fold0_feats.append(f0)

            # global MinMax on fold-0 features
            all_f0       = np.vstack(fold0_feats)
            feat_scaler  = MinMaxScaler().fit(all_f0)

            # train FAMs on scaled fold-0 features
            for (_, fam_m, _), f0 in zip(ensemble, fold0_feats):
                fam_m.train(feat_scaler.transform(f0), y0, epochs=1)

            # online test/train folds 1–9
            run_metrics = []
            for i in range(1, 10):
                Xi, yi = scaler0.transform(X_folds[i]), y_folds[i]

                preds = []
                for ann_m, fam_m, ext in ensemble:
                    fi = ext.predict(Xi, verbose=0)
                    fi = feat_scaler.transform(fi)
                    preds.append(fam_m.predict(fi))
                maj = mode(np.stack(preds), axis=0).mode.flatten()

                run_metrics.append({
                    "Accuracy":  accuracy_score( yi, maj),
                    "Precision": precision_score(yi, maj, zero_division=0),
                    "Recall":    recall_score(   yi, maj, zero_division=0),
                    "F1":        f1_score(       yi, maj),
                    "AUROC":     roc_auc_score(  yi, maj),
                    "MCC":       matthews_corrcoef(yi, maj)
                })

                # train on this fold
                for ann_m, fam_m, ext in ensemble:
                    ann_m.fit(Xi, yi, epochs=20, batch_size=32, verbose=0)
                    tr = ext.predict(Xi, verbose=0)
                    fam_m.train(feat_scaler.transform(tr), yi, epochs=1)

            per_run_avgs.append(pd.DataFrame(run_metrics).mean())

        combo_avg = pd.concat(per_run_avgs, axis=1).T.mean()
        grid_results.append({
            "alpha": round(alpha, 2),
            "rho":   round(rho,   2),
            **combo_avg.to_dict()
...
ts      = datetime.now().strftime("%Y%m%d_%H%M%S")
out_g   = os.path.expanduser(f"~/Desktop/GridSearch_noErrorPool_{ts}.xlsx")
grid_df.to_excel(out_g, index=False)
print(f"\n✅ Grid search (no error pool) saved to: {out_g}")'''